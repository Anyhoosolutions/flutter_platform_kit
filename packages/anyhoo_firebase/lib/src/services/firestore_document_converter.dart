...
Map<String, dynamic>? fromFirestoreDocument(Map<String, dynamic>? data, String id, {String? documentPath}) {
...
Map<String, dynamic> toFirestoreDocument(Map<String, dynamic> data) {
  return _convertDocument(
    convert: () => {for (final entry in data.entries) entry.key: _toFirestoreValue(entry.value, path: entry.key)},
  );
}

Map<String, dynamic> _convertDocument({required Map<String, dynamic> Function() convert}) {
  try {
    return convert();
  } on FirestoreConversionException {
    rethrow;
  } catch (e) {
    throw FirestoreConversionException(message: 'Failed to convert document', cause: e);
  }
}

dynamic _fromFirestoreValue(dynamic value, {required String path, String? documentPath}) {
...
dynamic _toFirestoreValue(dynamic value, {required String path, String? documentPath}) {
  return _convertValue(
    value,
    path: path,
    documentPath: documentPath,
...
T _convertValue<T>(dynamic value, {required String path, String? documentPath, required T Function() convert}) {
  try {
    return convert();
  } on FirestoreConversionException {
    rethrow;
