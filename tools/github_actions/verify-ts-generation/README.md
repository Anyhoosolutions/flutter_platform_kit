...
- ✅ Quick check mode for WIP PRs (only checks changed files, doesn't run generation)
...
For WIP PRs, use quick check mode which only verifies that changed Dart models have matching generated TypeScript files without running the full generation process:
...
| `quick_check` | Quick check mode: only verify changed Dart models have matching generated files (doesn't run generation) | No | `false` |

## What This Action Does

### Quick Check Mode (`quick_check: true`)

When `quick_check` is enabled, the action:
1. **Finds Changed Dart Models** - Identifies Dart model files that changed in the PR
2. **Verifies Generated Files Exist** - Checks that each changed Dart file has a corresponding generated TypeScript file
3. **Verifies Files Were Updated** - Ensures the generated TypeScript files were also changed in the PR
4. **Fails if Missing/Out of Sync** - Fails if generated files are missing or weren't updated

This mode is faster and doesn't require setting up Flutter/Node/Java or running the generation script. Perfect for WIP PRs.
