    ...
    for (final declaration in compilationUnit.declarations) {
      ...
      for (final constructor in factoryConstructors) {
        ...
        for (final param in constructor.parameters.parameters) {
            ...
            if (annotation.name.name == 'JsonKey' && annotation.arguments != null) {
              ...
              for (final arg in annotation.arguments!.arguments) {
                if (arg is NamedExpression) {
                  ...
                  } else if (argName == 'ignore' && arg.expression is BooleanLiteral) {
          ...
          if (finalTsType.contains('Timestamp')) {
        ...
        for (final directive in compilationUnit.directives) {
          if (directive is ImportDirective) {
            ...
            if (dartImportPath != null && dartImportPath.startsWith('package:')) {
              ...
              if (tempParts.isNotEmpty && _ignoredPackages.contains(tempParts.first)) {
                continue;
              }
    ...
    switch (dartType) {
      ...
      default:
        ...
        if (dartType.startsWith('Map<')) {
          ...
          final innerPart = dartType.substring(4, dartType.length - 1);
          final commaIndex = innerPart.indexOf(',');
          if (commaIndex != -1) {
            final valueType = innerPart.substring(commaIndex + 1).trim();
            final convertedValueType = _dartToTsType(valueType);
            // For now, assume all map keys are strings (most common case)
            return '{ [key: string]: $convertedValueType }';
          }
          return '{ [key: string]: any }'; // Fallback if parsing fails
        }
